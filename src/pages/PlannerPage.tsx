...
const PlannerPage = () => {
  ...
  return (
    <ModuleLayout title={t("planner.title")} subtitle={t("planner.subtitle")} icon={BookOpen}>
      <div className="mx-auto grid max-w-4xl gap-6 lg:grid-cols-2">
        <Card className="space-y-4 p-6 opacity-0 animate-fade-up" style={{ animationDelay: "100ms" }}>
          <label className="text-sm font-semibold text-foreground">{t("planner.inputLabel")}</label>
          <Textarea
            value={input}
        ...
        <Card className="p-6 opacity-0 animate-fade-up" style={{ animationDelay: "200ms" }}>
          <div className="flex items-center justify-between">
            <label className="text-sm font-semibold text-foreground">{t("planner.resultLabel")}</label>
            {result && (
